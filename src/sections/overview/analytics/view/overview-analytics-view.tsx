...
import { CustomHandsontable } from '../custom-handsontable';
...
export function OverviewAnalyticsView() {
  ...
  const changeTrueFalseOptions = (option: string, value: boolean) => {
    const temp = { ...myOptions };
    temp.trueFalseOptions[option as keyof typeof temp.trueFalseOptions] = !value;
    setMyOptions(temp);
  ...
  const makeTrueFalseCheckBox = () => {
    ...
    return pair.map((item, idx) => (
      <FormControlLabel
        key={idx}
        control={<Checkbox checked={item[1] as boolean} />}
        label={item[0]}
        onChange={() => changeTrueFalseOptions(item[0], item[1] as boolean)}
      />
    ));
  };

  const changeNumberOptions = (option: string, value: string) => {
    const temp = { ...myOptions };

    if (isNaN(Number(value))) return;

    temp.numberOptions[option as keyof typeof temp.numberOptions] = Number(value);
    setMyOptions(temp);
    localStorage.setItem(MY_OPTIONS, JSON.stringify(temp));
  };
  ...
  const makeNumberInput = () => {
    ...
    return pair.map((item, idx) => (
      <FormControl key={idx} sx={{ m: 2 }} variant="standard">
        <InputLabel htmlFor={`input-${item[0]}`}>{item[0]}</InputLabel>
        <Input
          id={`input-${item[0]}`}
          value={item[1]}
  ...
  const list = () => (
    <Box sx={{ width: 600 }}>
      ...
      <Box sx={{ m: 2, flexDirection: 'row' }}>
        <FormHelperText sx={{ color: 'blue' }}>0 이상 숫자를 입력하세요.</FormHelperText>
        {makeNumberInput()}
      </Box>
    </Box>
  );
  ...
  const initLocalStorage = () => {
    const localOptions = localStorage.getItem(MY_OPTIONS);
  ...
  return (
    <DashboardContent maxWidth="xl">
      <CustomHandsontable myOptions={myOptions} />
    </DashboardContent>
  );
}
